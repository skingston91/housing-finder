...
4. **Commute** — **Partial:** TfL Unified API for **transit**; **OpenRouteService** directions for **driving** / **cycling** / **walking** when **`ORS_API_KEY`** is set on `SearchAreasFunction` (`shared/commute/orsDirections.ts`, in-memory cache). **Next:** OSRM self-host, Google Directions, Secrets Manager, stricter rate limits.
