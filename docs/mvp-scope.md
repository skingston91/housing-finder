...
2. **Land Registry** — **Partial:** live **UK HPI** borough **average** prices via linked-data JSON + **6h** in-Lambda cache (`resolveLondonBoroughMedianRows`, `UKHPI_LIVE` on `SearchAreasFunction`). Static table fallback. **Next:** SPARQL single-query refresh, property-type-specific HPI measures.
