...
- **Implementation:** `shared/affordability/londonBoroughMedians.ts` keeps **borough names and representative centroids** (nearest-borough matching). **Prices:** when **`UKHPI_LIVE`** is enabled on `SearchAreasFunction` (default in `template.yaml`; set to **`0`** to disable), `shared/affordability/resolveLondonBoroughMedianRows.ts` loads the latest **UK HPI average price** per borough via **one SPARQL POST** to `https://landregistry.data.gov.uk/landregistry/query` (`shared/affordability/ukhpiSparql.ts`), then **per-borough linked-data JSON** (`shared/affordability/ukhpiLinkedDataApi.ts`) only for boroughs missing from the SPARQL result. **Caches 6 hours** per Lambda instance. On total failure, scoring falls back to static medians in-repo. Optional **max £/m²** is still blended in `affordabilityScoreForAreaSearch`. Metadata: `affordabilityPriceSource`, `ukhpiRefMonth`, `landRegistryOgl`. **Note:** UK HPI publishes **averages**, not medians — the score remains a transparent heuristic.
