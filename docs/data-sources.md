...
- **Implementation:** `shared/affordability/londonBoroughMedians.ts` keeps **borough names and representative centroids** (nearest-borough matching). **Prices:** when **`UKHPI_LIVE`** is enabled on `SearchAreasFunction` (default in `template.yaml`; set to **`0`** to disable), `shared/affordability/resolveLondonBoroughMedianRows.ts` loads the latest **UK HPI average price** per borough via the linked-data **JSON** API (`shared/affordability/ukhpiLinkedDataApi.ts` — two GETs per borough: region index + latest month observation), merges into those rows, and **caches 6 hours** per Lambda instance. On total failure, scoring falls back to static medians in-repo. Optional **max £/m²** is still blended in `affordabilityScoreForAreaSearch`. Metadata: `affordabilityPriceSource`, `ukhpiRefMonth`, `landRegistryOgl`. **Note:** UK HPI publishes **averages**, not medians — the score remains a transparent heuristic.
