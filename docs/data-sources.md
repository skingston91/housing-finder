...
- **Ranking when routing fails:** If TfL or OpenRouteService was invoked but returned no usable journey, the area gets metadata **`commuteRankTier: 1`**, an extra commute subscore penalty (`shared/commute/commuteScoreNetworkRoutingBonus.ts`), and is sorted **after** areas with a confirmed route (`commuteRankTier: 0`).
