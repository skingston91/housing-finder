...
- **Strict routing (production):** `SearchAreasFunction` env **`SEARCH_AREAS_ROUTING_STRICT`** (`template.yaml` **Parameter** `SearchAreasRoutingStrict`). When **`1`**, `POST /api/search-areas` returns **400** if the request’s commute mode needs **TfL** (transit) or **OpenRouteService** (drive/cycle/walk) but the corresponding key is missing — no silent straight-line fallback. Default **`0`** keeps local SAM usable without keys. Production deploys should set **`1`** once both keys are configured. Validation: `shared/searchAreas/validateSearchAreasRoutingKeys.ts`.
